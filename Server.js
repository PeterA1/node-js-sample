...
function buildObj(data,index,Arr) {
    ...
    for ( var i = 0; i < data.length; i++) {
        ...
        for ( var a = index+2; a < Arr.length; a++ ) {
            ...
            if ( data[i].hasOwnProperty(Arr[a]) ) { newObj[Arr[a]] = data[i][Arr[a]]; data[i] = data[i][Arr[a]]; newObj = newObj[Arr[a]];} else { console.log("No Own Prop",Arr[a],data[i]); } //if ( data[i].hasOwnProperty('0') )}
            //data[i] = data[i][Arr[a]];
