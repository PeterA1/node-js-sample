...
function MessageRecieved(ws,message) {
    ...
    if ( D[0] == 'MYU' ) { console.log(ws.id,D[1]); for ( var i = 0; i < Traffic.length; i++ ) { if ( Traffic[i].id == D[1]) { console.log("Removing from Traffic",D[1]); Traffic.splice(i,1); } ws.id = D[1]; Traffic.push(ws); return; } }
