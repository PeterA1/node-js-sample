...
function buildObj(data,index,Arr) {
    var returnArr = [];
    //console.log("Array is",data.length);
    for ( var i = 0; i < data.length; i++) {
        ...
        for ( var a = index+2; a < Arr.length; a++ ) {
            ...
            if ( data[i].hasOwnProperty(Arr[a]) ) { newObj[Arr[a]] = data[i][Arr[a]]; data[i] = data[i][Arr[a]]; newObj = newObj[Arr[a]];} else { if ( Array.isArray(data[i]) && data[i][0].hasOwnProperty(Arr[a]) ) { newObj = []; for ( var kk = 0; kk < data[i].length; kk++ ) { newObj.push(data[i][kk][Arr[a]]); } } else {  newObj = 'PAundefined'; } } } //if ( data[i].hasOwnProperty('0') )}
            //data[i] = data[i][Arr[a]];
