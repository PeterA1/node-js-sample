...
function downselect(ws,string,options,selected) {
    ...
    if ( string.indexOf("<?xml version") > -1 || string.indexOf('xmlns=') > -1 ) {
        ...
        //console.log(string);
        for ( var kk = 0; kk < data.response.listing.length; kk++ ) { for ( var pq in data.response.listing[kk] ) { if ( pq == 'floor_plan' ) { console.log(pq,data.response.listing[kk][pq]); } else { console.log(pq); } } }
    }
    else {
